...
$basePath = $basePath ?? "/eventos"; // caso não venha da view
...
$primeiroW = (int)(new DateTime($inicio->format('Y-m-01')))->format('w'); // 0=dom

/* Buscar eventos do mês */
$pdo = Database::getConnection();
$stmt = $pdo->prepare("SELECT data_evento, periodo, status FROM events WHERE data_evento BETWEEN :ini AND :fim");
...
function dayBadgeCal($ymd,$ocupado){
  $p1=!empty($ocupado[$ymd]['P1']); $p2=!empty($ocupado[$ymd]['P2']);
...
<div class="border rounded-3 p-3">
  ...
  <div class="d-flex justify-content-between align-items-center mb-2">
    <a class="btn btn-sm btn-outline-secondary"
       href="<?= $basePath . '?mes=' . (clone $inicio)->modify('-1 month')->format('Y-m') ?>">
